...
def fill(p, w):
    for t in w.split(','):
        ...
        if direction == 'R':
            p.extend((x+v+1, y) for v in range(value))
        elif direction == 'L':
            p.extend((x-v-1, y) for v in range(value))
        elif direction == 'U':
            p.extend((x, y+v+1) for v in range(value))
        elif direction == 'D':
            p.extend((x, y-v-1) for v in range(value))
        else:
            raise
...
p1 = fill([(0, 0)], text.splitlines()[0])
p2 = fill([(0, 0)], text.splitlines()[1])

inter = set(p1) & set(p2)
inter.remove((0, 0))

res = min(inter, key=lambda t: abs(t[0]) + abs(t[1]))
...
r = {p: (q1[p], q2[p]) for p in inter}
res = r[min(r, key=lambda p: sum(r[p]))]
print(abs(res[0]) + abs(res[1]))

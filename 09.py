...
class Amplifier:
    def __init__(self, array, total_memory=10000):
        self.array = list(array).copy() + [0] * (total_memory - len(array))
        self.ip = 0
        self.rb = 0
        self.input = deque()
        self.output = None
    ...
    def run(self):
        while True:
            ...
            instruction = "%05d" % self.array[self.ip]
            ...
            for k in range(nparam):
                if modes[k] == 0:
                    indices.append(self.array[self.ip+k+1])
                elif modes[k] == 1:
                    indices.append(self.ip+k+1)
                elif modes[k] == 2:
                    indices.append(self.array[self.ip+k+1] + self.rb)

            if opcode == 99:
                break
            elif opcode == 1:
                self.array[indices[2]] = self.array[indices[1]] + self.array[indices[0]]
            elif opcode == 2:
                self.array[indices[2]] = self.array[indices[1]] * self.array[indices[0]]
            elif opcode == 3:
                self.array[indices[0]] = self.input.popleft()
            elif opcode == 4:
                self.output = self.array[indices[0]]
            elif opcode == 5:
                if self.array[indices[0]] != 0:
                    self.ip = self.array[indices[1]]
                ...
                self.array[indices[2]] = 1 if self.array[indices[0]] < self.array[indices[1]] else 0
            elif opcode == 8:
                self.array[indices[2]] = 1 if self.array[indices[0]] == self.array[indices[1]] else 0
            elif opcode == 9:
                self.rb += self.array[indices[0]]
...
# 1
print(Amplifier(array).add_inputs(1).run())

# 2
print(Amplifier(array).add_inputs(2).run())
